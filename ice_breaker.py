...
if __name__ == '__main__':
    ...
    # Information refers to the {information within the summary template}
    summary_prompt_template = PromptTemplate(
        input_variables=["information"], template=summary_template
    )

    # temperature decides how creative the language model will be. 0 means it won't be creative
    # llm = ChatOpenAI(temperature=0, model_name="gpt-3.5-turbo")

    # Changing llm to use llama3
    # llm = ChatOllama(temperature=0, model="llama3")

    # Changing llm to use mistral
    # llm = ChatOllama(temperature=0, model="mistral")

    llm = ChatOllama(model="mistral")

    # tying everything together with a chain
    # It takes the details in the summary_prompt_template and sends it to llm
    chain = summary_prompt_template | llm | StrOutputParser()

    # Now to run this chain we use the chain.invoke method
